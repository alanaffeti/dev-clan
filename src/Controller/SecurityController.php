...
class SecurityController extends AbstractController
{
    ...
    public function login(AuthenticationUtils $authenticationUtils): Response
    {
        ...
        $user = $this->getDoctrine()->getRepository(User::class)->findOneBy(['name' => $lastUsername]);
        if ($user && $user->getBlock()) {
            throw new AuthenticationException('This user has been blocked.');
        }
    ...
    public function forgotPassword(Request $request, UserRepository $userRepository,Swift_Mailer $mailer, TokenGeneratorInterface  $tokenGenerator)
    {
        ...
        if($form->isSubmitted() && $form->isValid()) {
            ...
            $message = (new Swift_Message('Mot de password oublié'))
                ->setFrom('khlifaoui.emna@esprit.tn')
                ->setTo($user->getEmail())
